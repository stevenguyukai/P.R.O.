...
public class userInfoDaoImplement extends BaseDAO implements userInfoDao{
    ...
    public boolean getUsersInformation(userInfo userInfo){
        ...
        String sql1 = "insert into itemAmount(userID,itemID,location) values(?,?,?)";
        int isRight  = jdbcTemplate.update(sql,sql1,userInfo.getName(),userInfo.getAvatar(),userInfo.getOPENID(),userInfo.getAvailable(),userInfo.getResentLongitude(),userInfo.getResentLatitude(),userInfo.getResentRegion(),userInfo.getResentLoginTime(),userInfo.getComment(),userInfo.getOPENID(),1000,userInfo.getResentRegion());
        return isRight==1;
    }
