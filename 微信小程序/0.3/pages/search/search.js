...
Page({
  ...
  onLoad: function(options) {
   ...
   //搜索引擎
    ...
    if (cardTeamsGet[22].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[22]),
      })
    }
    if (cardTeamsGet[23].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[23]),
      })
    }
    if (cardTeamsGet[24].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[24]),
      })
    }
    if (cardTeamsGet[25].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[25]),
      })
    }
    if (cardTeamsGet[26].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[26]),
      })
    }
    if (cardTeamsGet[27].name.indexOf(searchText) >= 0) {
      this.setData({
        cardTeams: this.data.cardTeams.concat(cardTeamsGet[27]),
      })
    }
