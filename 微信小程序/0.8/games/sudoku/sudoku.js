...
function updateInfoFunction(that) {
  ...
  if (app.globalData.isGettedInfo == true) {
    ...
    if (middle) {
      addExp = 125
      brainScoreIncreaseRate = 1.2
    }
    if (hard) {
      addExp = 175
      brainScoreIncreaseRate = 1.5
    }
    if (epic) {
      addExp = 250
      brainScoreIncreaseRate = 2
    }

    //更新各个脑力值数据
    app.updateBrainScore("Observation", 22 * brainScoreIncreaseRate)
    app.updateBrainScore("Focus", 28 * brainScoreIncreaseRate)
    app.updateBrainScore("Logical", 16 * brainScoreIncreaseRate)
