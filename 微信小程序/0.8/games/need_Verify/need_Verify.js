...
function summaryPageAnimation(that) {
    ...
    if (app.globalData.isGettedInfo == true) {
        ...
        //设置每种模式的经验值
        if (gameMode == "easy") {
            addExp = 15 + that.data.errorNum * 0 + that.data.correctNum * 15 + that.data.missNum * 3
            brainScoreIncreaseRate = 1
        }
        if (gameMode == "middle") {
            addExp = 15 + that.data.errorNum * 0 + that.data.correctNum * 20 + that.data.missNum * 5
            brainScoreIncreaseRate = 1.15
        }
        if (gameMode == "hard") {
            addExp = 25 + that.data.errorNum * 0 + that.data.correctNum * 40 + that.data.missNum * 8
            brainScoreIncreaseRate = 1.56
        }
        if (gameMode == "epic") {
            addExp = 30 + that.data.errorNum * 0 + that.data.correctNum * 50 + that.data.missNum * 13
            brainScoreIncreaseRate = 1.8
        }

        //更新各个脑力值数据
        app.updateBrainScore("Memory", 13 * brainScoreIncreaseRate + 1 * that.data.correctNum - 1.3 * that.data.errorNum * brainScoreIncreaseRate)
        app.updateBrainScore("Focus", 8 * brainScoreIncreaseRate + 1 * that.data.correctNum - 1.1 * that.data.errorNum * brainScoreIncreaseRate)
        app.updateBrainScore("React", 7 * brainScoreIncreaseRate + 1 * that.data.correctNum - 0.8 * that.data.errorNum * brainScoreIncreaseRate)
        ...
        setTimeout(() => {
            if (isFull) {
                ...
                app.globalData.need_verify_play_time = (app.globalData.need_verify_play_time + play_time),
...
Page({
    ...
    getVerifyCode() {
        ...
        if (that.data.passwordNeed) {
            ...
            that.setData({
                passwardTelling: "密码为" + password + "(注意大小写)，"
            })
        }
